...
import logging
import torch
from models.modules.utils.plot_metrics import get_mean_max_metric
from .optimization import BertAdam
import json
from models.modules.data.bert_data import BertNerData
from models.modules.models.released_models import released_models


logging.basicConfig(level=logging.INFO)

stop_labels = ['<pad>', '[CLS]', 'X']
...
def validate_step(dl, model, id2label, sup_labels, id2cls=None):
    ...
    fact_labels = sorted(set([t for s in targets_cpu for t in s if t not in stop_labels]))
    sup_labels = [l for l in sup_labels if l in fact_labels]
    ...
    return clf_report, targets_cpu, preds_cpu
    ...
    def fit_one_cycle(self, epoch, target_metric="f1"):
        ...
        if epoch % self.validate_every == 0:
            if self.data.is_cls:
                ...
                rep, _, _ = validate_step(self.data.valid_dl, self.model, self.data.id2label, self.sup_labels)
