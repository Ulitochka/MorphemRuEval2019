...
from models.modules.data import tokenization
import torch
import logging
import pandas as pd
import numpy as np
...
def get_data(
    ...
    for args in tqdm_notebook(enumerate(zip(*all_args)), total=total, leave=False):
        ...
        if len([el for el in bert_labels if el not in ['X', '[CLS]']]) != len(labels):
            log_str = 'Corrupted seq. Origin_tokens: %s; Origin_labels: %s; Ber_labels: %s;' % (
                str(orig_tokens), str(labels), str([el for el in bert_labels if el not in ['X', '[CLS]']]))
            logging.info(log_str)

        for el in bert_labels:
...
def get_bert_data_loaders(train, valid, vocab_file, batch_size=16, cuda=True, is_cls=False,
    ...
    train_dl = DataLoaderForTrain(train_f, batch_size=batch_size, shuffle=True, cuda=cuda)
    ...
    valid_dl = DataLoaderForTrain(valid_f, batch_size=batch_size, cuda=cuda, shuffle=False)
