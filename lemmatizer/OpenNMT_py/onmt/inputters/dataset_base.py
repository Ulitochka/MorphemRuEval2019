...
class DatasetBase(Dataset):
    ...
    The resulting dataset will have three attributes_parser (todo: also src_vocabs):

     `examples`: a list of `torchtext.data.Example` objects with attributes_parser as
        described above.
     `fields`: a dictionary whose keys are strings with the same names as the
        attributes_parser of the elements of `examples` and whose values are
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter,
        ...
        # the dataset's self.fields should have the same attributes_parser as examples
