...
class Translator(object):
    """
    Uses a models to translate a batch of sentences.


    Args:
       model (:obj:`onmt.modules.NMTModel`):
          NMT models to use for translation
    ...
    def translate(
        ...
        for batch in data_iter:
            ...
            for index_trans in range(len(translations)):
                all_scores += [translations[index_trans].pred_scores[:self.n_best]]
                pred_score_total += translations[index_trans].pred_scores[0]
                pred_words_total += len(translations[index_trans].pred_sents[0])
                if tgt is not None:
                    gold_score_total += translations[index_trans].gold_score
                    gold_words_total += len(translations[index_trans].gold_sent) + 1

                n_best_preds = [" ".join(pred)
                                for pred in translations[index_trans].pred_sents[:self.n_best]]
                all_predictions += [n_best_preds]

                if use_sent_borders:
                    output_file_string = "_	%s	%s	%s	_	_	_	_	_	_" % (
                        ''.join(src[index_trans].split()[:-1]),
                        ''.join(n_best_preds[0].split()),
                        src[index_trans].split()[-1])
                    self.out_file.write(output_file_string + '\n')
                    self.out_file.flush()
                else:
                    self.out_file.write('\n'.join(n_best_preds) + '\n')
                    self.out_file.flush()
                ...
                if self.verbose:
                    ...
                    output = translations[index_trans].log(sent_number)
                    if self.logger:
                ...
                if attn_debug:
                    preds = translations[index_trans].pred_sents[0]
                    preds.append('</s>')
                    attns = translations[index_trans].attns[0].tolist()
                    if self.data_type == 'text':
                        srcs = translations[index_trans].src_raw
                    else:
