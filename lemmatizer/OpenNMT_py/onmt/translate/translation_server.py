...
class TranslationServer():
    ...
    def clone_model(self, model_id, opt, timeout=-1):
        ...
        if model_id in self.models:
            ...
            raise ServerModelError("No such models '%s'" % str(model_id))

    def load_model(self, opt, model_id=None, **model_kwargs):
        """Loading a models given a set of options
        """
    ...
    def preload_model(self, opt, model_id=None, **model_kwargs):
        """Preloading the models: updating internal datastructure
           It will effectively load the models if `load` is set
        """
        ...
        print("Pre-loading models %d" % model_id)
    ...
    def run(self, inputs):
        """Translate `inputs`
           ...
           We use inputs[0]["id"] as the models id
        """
        model_id = inputs[0].get("id", 0)
        if model_id in self.models and self.models[model_id] is not None:
            return self.models[model_id].run(inputs)
        else:
            print("Error No such models '%s'" % str(model_id))
            raise ServerModelError("No such models '%s'" % str(model_id))

    def unload_model(self, model_id):
        """Manually unload a models.
...
class ServerModel:
    def __init__(self, opt, model_id, tokenizer_opt=None, load=False,
        ...
        """
            Args:
                ...
                load: (bool) whether to load the models during __init__
                ...
                model_root: (str) path to the models directory
                            it must contain de models and tokenizer file
    ...
    def load(self):
        ...
        timer = Timer()
        self.logger.info("Loading models %d" % self.model_id)
        timer.start()
        ...
        if self.tokenizer_opt is not None:
            ...
            if self.tokenizer_opt['type'] == 'sentencepiece':
                if "models" not in self.tokenizer_opt:
                    raise ValueError(
                        "Missing mandatory tokenizer option 'models'")
                import sentencepiece as spm
                sp = spm.SentencePieceProcessor()
                model_path = os.path.join(self.model_root,
                                          self.tokenizer_opt['models'])
                sp.Load(model_path)
                self.tokenizer = sp
    ...
    @critical
    def run(self, inputs):
        """Translate `inputs` using this models
        ...
        if len(texts_to_translate) > 0:
            try:
                ...
                self.logger.error("models: #%s" % self.model_id)
                self.logger.error("models opt: " + str(self.opt.__dict__))
                self.logger.error(traceback.format_exc())

                raise ServerModelError(err)

        timer.tick(name="translation")
        self.logger.info("""Using models #%d\t%d inputs
    ...
    def do_timeout(self):
        """Timeout function that free GPU memory by moving the models to CPU
           or unloading it; depending on `self.on_timemout` value
        """
        if self.on_timeout == "unload":
            self.logger.info("Timeout: unloading models %d" % self.model_id)
            self.unload()
        if self.on_timeout == "to_cpu":
            self.logger.info("Timeout: sending models %d to CPU"
                             % self.model_id)
            self.to_cpu()

    @critical
    def unload(self):
        self.logger.info("Unloading models %d" % self.model_id)
        del self.translator
        if self.opt.cuda:
    ...
    @critical
    def to_cpu(self):
        """Move the models to CPU and clear CUDA cache
        """
